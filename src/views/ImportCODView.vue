<template>
  <div class="container mx-auto max-w-7xl px-4 py-8">
    <h1 class="mb-8 text-2xl font-bold text-gray-800">นำเข้า COD (Preview Mode)</h1>

    <!-- Upload Section -->
    <div class="mb-8 rounded-xl bg-white p-8 shadow-md border border-gray-100 text-center">
      <div 
      ...
      >
        ...
        >
          <div class="flex flex-col items-center justify-center pt-5 pb-6">
            <svg class="mb-4 h-10 w-10 text-gray-400" :class="{ 'text-blue-500': isDragging }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
            </svg>
            <p v-if="!isDragging" class="mb-2 text-sm text-gray-500"><span class="font-semibold">คลิกเพื่อเลือกไฟล์</span> หรือลากไฟล์มาวางที่นี่</p>
            <p v-else class="mb-2 text-sm text-blue-600 font-semibold">วางไฟล์ลงที่นี่ได้เลย</p>
            <p class="text-xs text-gray-500">รองรับไฟล์ .xlsx (Filename format: YYYYMMDD_...)</p>
          </div>
          <input 
            type="file" 
            class="hidden" 
    ...
    <!-- Summary Stats -->
    <div v-if="previewItems.length > 0" class="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <div class="rounded-xl bg-blue-50 p-6 border border-blue-100">
        <h3 class="text-sm font-medium text-blue-800">จำนวนไฟล์</h3>
        <p class="mt-2 text-3xl font-bold text-blue-900">{{ processedFilesCount }} ไฟล์</p>
      </div>
      <div class="rounded-xl bg-green-50 p-6 border border-green-100">
        ...
        <p class="mt-2 text-3xl font-bold text-green-900">{{ previewItems.length }} รายการ</p>
      </div>
      <div class="rounded-xl bg-purple-50 p-6 border border-purple-100">
        ...
        <p class="mt-2 text-3xl font-bold text-purple-900">฿{{ formatCurrency(totalAmount) }}</p>
      </div>
    </div>
    ...
    <div v-if="previewItems.length > 0" class="mb-8 flex justify-center gap-4">
      ...
      >
        <svg v-if="isSaving" class="mr-3 h-6 w-6 animate-spin" fill="none" viewBox="0 0 24 24">
    ...
    <!-- Preview Table -->
    <div v-if="previewItems.length > 0" class="rounded-xl bg-white shadow-sm border border-gray-200 overflow-hidden">
      <div class="border-b border-gray-200 bg-gray-50 px-6 py-4">
        <h3 class="text-lg font-medium text-gray-900">ตัวอย่างข้อมูล (Preview)</h3>
      </div>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">ลำดับ</th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">วันที่ (จากไฟล์/ชื่อไฟล์)</th>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Customer Name</th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Phone / Address</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in previewItems" :key="index" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ index + 1 }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ formatDate(item.date) }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ item.orderNo }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ item.customerName }}</td>
              <td class="px-6 py-4 text-xs text-gray-500">
                <div v-if="item.phoneNumber" class="flex items-center gap-1">📞 {{ item.phoneNumber }}</div>
                <div v-if="item.address" class="truncate max-w-xs" :title="item.address">📍 {{ item.address }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">฿{{ formatCurrency(item.amount) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (!worksheet) {
            Swal.fire({ icon: 'error', title: 'ไม่พบข้อมูล', text: `ไฟล์ ${file.name} ไม่พบ Worksheet` })
            continue
        }
        ...
        worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
            ...
            if (headerRowIndex === -1) {
                row.eachCell((cell, colNumber) => {
                    ...
                    if (val.includes('order') && val.includes('no')) colMap.orderNo = colNumber
                    if (val.includes('recipient') || val.includes('name')) {
                         if (colMap.customerName === -1) colMap.customerName = colNumber
                    }
                    if (val.includes('cod') && val.includes('amount')) colMap.amount = colNumber
                    // Check for Tracking No
                    if (val.includes('tracking') && val.includes('no')) colMap.trackingNo = colNumber
                    // Check for Phone
                    if (val.includes('phone') || val.includes('tel') || val.includes('mobile') || val.includes('เบอร์')) colMap.phoneNumber = colNumber
                    // Check for Address
                    if (val.includes('address') || val.includes('ที่อยู่')) colMap.address = colNumber
                    // Check for Picked up DATE
                    if (val.includes('picked') && val.includes('date')) colMap.pickedUpDate = colNumber
                })

                // Validate Header Found (Must have Order or Tracking AND Amount)
                if ((colMap.orderNo !== -1 || colMap.trackingNo !== -1) && colMap.amount !== -1) {
            ...
            if (rowNumber > headerRowIndex) {
                 const orderCell = colMap.orderNo !== -1 ? row.getCell(colMap.orderNo) : null
                 const trackingCell = colMap.trackingNo !== -1 ? row.getCell(colMap.trackingNo) : null
                 const nameCell = colMap.customerName !== -1 ? row.getCell(colMap.customerName) : null
                 const amountCell = row.getCell(colMap.amount)
                 const phoneCell = colMap.phoneNumber !== -1 ? row.getCell(colMap.phoneNumber) : null
                 const addressCell = colMap.address !== -1 ? row.getCell(colMap.address) : null
                 const dateCell = colMap.pickedUpDate !== -1 ? row.getCell(colMap.pickedUpDate) : null

                 let orderNoVal = orderCell && orderCell.text ? orderCell.text.trim() : ''
                 let trackingNoVal = trackingCell && trackingCell.text ? trackingCell.text.trim() : ''
                 ...
                 let customerName = nameCell && nameCell.text ? nameCell.text.trim() : ''
                 let amountStr = amountCell.text ? amountCell.text.trim() : '0'
                 
                 // Extract Phone & Address
                 let phoneNumber = phoneCell && phoneCell.text ? phoneCell.text.trim() : ''
                 let address = addressCell && addressCell.text ? addressCell.text.trim() : ''
                 ...
                 if (!hasLoggedFirstItem) {
                    console.log(`[${file.name}] First item extracted:`, { finalOrderNo, customerName, amountVal, itemDate })
        ...
        if (headerRowIndex === -1) {
             Swal.fire({ icon: 'error', title: 'ไม่พบหัวตาราง', text: `ไม่พบคอลัมน์ Order No / Tracking No และ Amount ในไฟล์ ${file.name}` })
        }

      } catch (err) {
        console.error(`Error processing ${file.name}:`, err)
        Swal.fire({ icon: 'error', title: 'อ่านไฟล์ล้มเหลว', text: `${file.name}: ${err.message}` })
      }
    }
  } catch (error) {
...
const confirmImport = async () => {
    ...
    const result = await Swal.fire({
        ...
        customClass: {
          confirmButton: 'bg-green-600 text-white font-bold py-2 px-6 rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 mx-2',
          cancelButton: 'bg-red-500 text-white font-bold py-2 px-6 rounded-lg shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 mx-2'
        }
    })
...
const parseDateFromFilename = (filename) => {
  ...
  if (!firstPart || firstPart.length !== 8 || isNaN(Number(firstPart))) return null
...
const formatDate = (date) => {
  if (!date) return '-'
  // Handle both Firestore Timestamp (if reloading) or JS Date (preview)
  const d = date && date.toDate ? date.toDate() : new Date(date)
  if (isNaN(d.getTime())) return '-'
  
  const yearBE = d.getFullYear() + 543
  // Show time if it's not 00:00:00 (implies extracted time)
  const hasTime = d.getHours() !== 0 || d.getMinutes() !== 0
  const timeStr = hasTime ? ` ${format(d, 'HH:mm')}` : ''
  
  return format(d, `d MMM ${String(yearBE).slice(-2)}`, { locale: th }) + timeStr
}
